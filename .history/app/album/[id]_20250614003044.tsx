...
export default function AlbumDetailScreen() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`https://sibi.iran.liara.run/api/v1/admin/album/${albumId}`).then(res => res.json()),
      fetch(`https://sibi.iran.liara.run/api/v1/admin/album/${albumId}/tracks`).then(res => res.json()),
    ])
      .then(([albumRes, tracksRes]) => {
        if (!albumRes.success) throw new Error(albumRes.message);
        if (!tracksRes.success) throw new Error(tracksRes.message);
        setAlbum(albumRes.payload);
        setTracks(tracksRes.payload);
      })
  ...
  if (error || !album) {
    return <View style={styles.center}><Text style={{ color: '#fff' }}>Error: {error || 'Album not found'}</Text></View>;
  }
